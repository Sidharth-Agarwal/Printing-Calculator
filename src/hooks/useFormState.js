...
/**
 * Custom hook for managing form field state with validation
 * Improved version to prevent input blocking and toggle issues
 */
const useFormState = (section) => {
  const { state, dispatch } = useBillingForm();
  const [errors, setErrors] = useState({});
  ...
  // Update a field in the section with local state first
  const updateField = useCallback((name, value) => {
    // Dispatch immediately but in a non-blocking way
    setTimeout(() => {
      dispatch({
        type: `UPDATE_${section.toUpperCase()}`,
        payload: { [name]: value }
