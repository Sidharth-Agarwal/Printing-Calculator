    ...
    <div className="border rounded p-3 mb-3 bg-gray-50 print:break-inside-avoid">
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div className="space-y-1 text-xs">
          ...
          <div><span className="font-medium">Provided:</span> <SafeValue value={estimate?.jobDetails?.paperProvided} fallback="No" /></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
        ...
        {estimate?.lpDetails?.isLPUsed && (
          <div className="bg-white p-2 rounded border">
            <h4 className="font-semibold mb-1">LP Details</h4>
            <div><span className="font-medium">Colors:</span> <SafeValue value={estimate.lpDetails.noOfColors} fallback="0" /></div>
            {estimate.lpDetails.colorDetails && estimate.lpDetails.colorDetails.length > 0 && (
              <div className="mt-1">
                {estimate.lpDetails.colorDetails.map((color, idx) => (
                  <div key={idx} className="bg-blue-50 p-1 rounded my-1">
                    <span className="font-medium">Color {idx + 1}:</span> <SafeValue value={color.pantoneType} />
                  </div>
                ))}
              </div>
            )}
        ...
        {estimate?.fsDetails?.isFSUsed && (
          <div className="bg-white p-2 rounded border">
            <h4 className="font-semibold mb-1">FS Details</h4>
            <div><span className="font-medium">Type:</span> <SafeValue value={estimate.fsDetails.fsType} /></div>
            {estimate.fsDetails.foilDetails && estimate.fsDetails.foilDetails.length > 0 && (
              <div className="mt-1">
                {estimate.fsDetails.foilDetails.map((foil, idx) => (
                  <div key={idx} className="bg-yellow-50 p-1 rounded my-1">
                    <span className="font-medium">Foil {idx + 1}:</span> <SafeValue value={foil.foilType} />
                  </div>
                ))}
              </div>
            )}
        ...
        {estimate?.embDetails?.isEMBUsed && (
          <div className="bg-white p-2 rounded border">
            ...
            <div>
              <span className="font-medium">Size:</span> 
              <SafeValue 
                value={`${estimate.embDetails.plateDimensions?.length || "?"} x ${estimate.embDetails.plateDimensions?.breadth || "?"}`}
                fallback="Not specified" 
              />
            </div>
            <div><span className="font-medium">Male Plate:</span> <SafeValue value={estimate.embDetails.plateTypeMale} /></div>
            <div><span className="font-medium">Female Plate:</span> <SafeValue value={estimate.embDetails.plateTypeFemale} /></div>
          </div>
        )}
        ...
        {estimate?.digiDetails?.isDigiUsed && (
          <div className="bg-white p-2 rounded border">
            <h4 className="font-semibold mb-1">Digital Details</h4>
            <div><span className="font-medium">Die:</span> <SafeValue value={estimate.digiDetails.digiDie} /></div>
            <div>
              <span className="font-medium">Size:</span> 
              <SafeValue 
                value={`${estimate.digiDetails.digiDimensions?.length || "?"} x ${estimate.digiDetails.digiDimensions?.breadth || "?"}`}
                fallback="Not specified"
              />
            </div>
          </div>
...
const GroupedJobTicket = ({ estimates, clientInfo, version, onRenderComplete }) => {
  ...
  useEffect(() => {
    if (onRenderComplete) {
      // Set a longer delay to ensure images have loaded
      const timer = setTimeout(() => {
        setIsReady(true);
        onRenderComplete();
      }, 1000);
