...
const EstimatesPage = () => {
  ...
  const [isGeneratingPDF, setIsGeneratingPDF] = useState(false);
  const [previewData, setPreviewData] = useState(null);
  const [pdfError, setPdfError] = useState(null); // New error state for PDF generation
  ...
  // Safely copy image data to avoid CORS issues
  const safelyLoadImage = async (imageUrl) => {
    // For images that could have CORS issues, use placeholder instead
    if (!imageUrl || imageUrl.startsWith('http')) {
      return null; // Will use fallback in GroupedJobTicket
    }
    
    return imageUrl; // Should be a data URL or local path
  };

  // Generate PDF for job ticket with improved error handling
  const handleGenerateJobTicket = async () => {
    if (!previewData || !previewData.estimates.length) return;
    ...
    try {
      // Create a temporary div outside the visible area
      const tempDiv = document.createElement('div');
      ...
      // Process images in the estimates to avoid CORS issues
      const processedEstimates = await Promise.all(
        previewData.estimates.map(async (estimate) => {
          // Deep clone the estimate to avoid modifying the original
          const clonedEstimate = JSON.parse(JSON.stringify(estimate));
          
          // If there's an image, try to process it
          if (clonedEstimate.dieDetails?.image) {
            const safeImage = await safelyLoadImage(clonedEstimate.dieDetails.image);
            if (clonedEstimate.dieDetails) {
              clonedEstimate.dieDetails.image = safeImage;
            }
          }
      ...
      // Create a new dataset with processed images
      const processedPreviewData = {
        ...previewData,
      ...
      await new Promise((resolve, reject) => {
        try {
          ...
          root.render(
            <GroupedJobTicket 
              estimates={processedPreviewData.estimates} 
              clientInfo={processedPreviewData.clientInfo}
              version={processedPreviewData.version}
              onRenderComplete={async () => {
                try {
                  ...
                  const canvas = await html2canvas(tempDiv, {
                    ...
                    logging: false,
                    allowTaint: true,
                    imageTimeout: 5000, // Increased timeout
                    onclone: (doc) => {
                      // Ensure all images are loaded or use fallbacks
                  ...
                  pdf.addImage(imgData, 'JPEG', 0, 0, imgWidth, imgHeight);
                  pdf.save(`${processedPreviewData.clientName}_V${processedPreviewData.version}_JobTicket.pdf`);
      ...
      setIsGeneratingPDF(false);
      // Clean up the temporary div - in a try/catch to prevent errors from stopping the function
      try {
        ...
        console.error('Error cleaning up temporary div:', e);
      }
    }
  };
  ...
  return (
    <div className="p-4 max-w-screen-xl mx-auto">
      ...
      {/* Preview Modal with improved error handling */}
      <PreviewModal
        isOpen={isPreviewOpen}
        onClose={() => setIsPreviewOpen(false)}
        onDownload={handleGenerateJobTicket}
      ...
      >
        {previewData && previewData.estimates.length > 0 ? (
          <div className="relative">
            {/* Add a loading overlay during PDF generation */}
            {isGeneratingPDF && (
              <div className="absolute inset-0 bg-white bg-opacity-75 flex items-center justify-center z-10">
                <div className="text-center">
                  <div className="animate-spin mb-4 h-12 w-12 border-4 border-blue-500 rounded-full border-t-transparent mx-auto"></div>
                  <p className="text-blue-600 font-medium">Generating PDF...</p>
          ...
          <div className="text-center py-8">
            <p className="text-gray-500">No estimate data available to preview</p>
          </div>
        )}
      </PreviewModal>
    </div>
