...
// // // //         {/* Circular Button */}
// // // //         <div
// // // //           className={`w-6 h-6 flex items-center justify-center border rounded-full transition ${
// // // //             isActive ? "border-blue-400 bg-blue-500" : "border-gray-300 bg-gray-200"
// // // //           }`}
// // // //         >
// // // //           {isActive && <div className="w-4 h-4 rounded-full bg-white"></div>}
...
// //       console.log(`Stage updated to "${stage}" for order ID: ${orderId}`);
// //     } catch (error) {
// //       console.error("Error updating stage in Firebase:", error);
// //       alert("Failed to update the stage. Please try again.");
// //     }
// //   };
...
//       // Update local state to reflect toggle change
//       setOrdersData((prevOrders) =>
//         prevOrders.map((order) =>
//           order.id === orderId ? { ...order, stage } : order
//         )
...
import { db } from "../../firebaseConfig";
import OrderDetailsModal from "./OrderDetailsModal"; // Import OrderDetailsModal
...
const OrdersPage = () => {
  ...
  const [noResults, setNoResults] = useState(false);
  const [sortCriteria, setSortCriteria] = useState("");
  const [selectedOrder, setSelectedOrder] = useState(null); // State for modal popup
  ...
  const handleSearch = (e) => {
    const query = e.target.value.toLowerCase();
    setSearchQuery(query);
    applyFiltersAndSort(query, sortCriteria);
  };

  const handleSortChange = (e) => {
    const selectedSort = e.target.value;
    setSortCriteria(selectedSort);
    applyFiltersAndSort(searchQuery, selectedSort);
  };

  const applyFiltersAndSort = (query, sortBy) => {
    let filtered = [...ordersData];
