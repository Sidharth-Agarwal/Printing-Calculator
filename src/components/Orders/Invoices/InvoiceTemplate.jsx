...
const InvoiceTemplate = ({ invoiceData, orders, clientInfo, totals }) => {
  ...
  const prepareLineItems = () => {
    return orders.map((order) => {
      ...
      const originalTotal =
        parseFloat(calculations.totalCost || costPerCard * quantity) || 0;

      // Loyalty discount from DB
      const loyaltyDiscountAmount = parseFloat(
        calculations.loyaltyDiscountAmount || 0
      );
  ...
  const recalculateTotals = () => {
    ...
    const totalLoyaltyDiscount = lineItems.reduce(
      (sum, item) => sum + item.loyaltyDiscountAmount,
      0
    );
    const totalInvoiceDiscount = lineItems.reduce(
      (sum, item) => sum + item.invoiceDiscountAmount,
      0
    );
  ...
  return (
    <div
      className="bg-white p-2 print:p-0"
      style={{ maxWidth: "600px", margin: "0 auto", fontSize: "65%" }}
    >
      ...
      <div className="flex justify-between mb-1">
        <div className="w-1/2">
          ...
          {/* Client Info */}
          <div>
            <h2 className="text-xs font-semibold text-gray-700 mb-0.5">
              Bill To:
            </h2>
        ...
        <div className="text-right w-1/2">
          ...
          <div className="my-0.5">
            <div className="text-xs">
              ...
              <div className="text-gray-600">A/C Holder Name: M/S FAMOUS</div>
              <div className="text-gray-600">A/C Number: 0160102000012661</div>
              <div className="text-gray-600">IFSC Code: IBKL0000160</div>
              <div className="text-gray-600">
                IDBI Bank, Circular Road, Dimapur
              </div>
            </div>
          </div>
        </div>
        ...
        >
          <thead>
            <tr className="bg-gray-100 text-gray-700">
              <th className="py-0.5 px-1 border border-gray-300 text-center w-6">
                S.No
              </th>
              <th className="py-0.5 px-1 border border-gray-300 text-center w-20">
                Order Serial
              </th>
              <th className="py-0.5 px-1 border border-gray-300 text-left w-18">
                Item
              </th>
              <th className="py-0.5 px-1 border border-gray-300 text-center w-12">
                Type
              </th>
              <th className="py-0.5 px-1 border border-gray-300 text-center w-18">
                Paper
              </th>
              <th className="py-0.5 px-1 border border-gray-300 text-center w-10">
                Qty
              </th>
              <th className="py-0.5 px-1 border border-gray-300 text-right w-14">
                Unit
              </th>
              <th className="py-0.5 px-1 border border-gray-300 text-right w-16">
                Total
              </th>
              <th className="py-0.5 px-1 border border-gray-300 text-center w-10">
                Disc%
          ...
          <tbody>
            {lineItems.map((item, index) => {
              const totalDiscount =
                item.loyaltyDiscountAmount + item.invoiceDiscountAmount;
              const discountPercent =
                item.originalTotal > 0
                  ? (totalDiscount / item.originalTotal) * 100
                  : 0;
              ...
              return (
                <tr key={item.id || index} className="text-gray-700">
                  ...
                  <td className="py-0.5 px-1 border border-gray-300 text-center font-mono text-[8px] whitespace-nowrap">
                    {item.orderId}
                  </td>
                  <td className="py-0.5 px-1 border border-gray-300 truncate">
                    <div className="font-medium truncate">{item.name}</div>
                  </td>
                  <td className="py-0.5 px-1 border border-gray-300 text-center truncate">
                    {item.jobType}
                  </td>
                  <td className="py-0.5 px-1 border border-gray-300 text-center truncate">
                    {item.paperInfo}
                  ...
                  <td className="py-0.5 px-1 border border-gray-300 text-right font-mono">
                  ...
                  <td className="py-0.5 px-1 border border-gray-300 text-right font-mono font-semibold">
                  ...
                  <td className="py-0.5 px-1 border border-gray-300 text-right font-mono font-bold">
        ...
        <table className="w-full border-collapse text-xs">
          <thead>
            <tr className="bg-gray-50">
              <th className="py-0.5 px-1 border border-gray-300 text-left">
                HSN Code
              </th>
              <th className="py-0.5 px-1 border border-gray-300 text-left">
                Job Types
          ...
          <tbody>
            {Object.entries(hsnSummary).map(([hsnCode, data], idx) => (
              <tr key={idx}>
                <td className="py-0.5 px-1 border border-gray-300 font-mono">
                  {hsnCode}
                </td>
