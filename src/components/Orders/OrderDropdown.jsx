...
const OrderDropdown = ({ order, onOrderUpdate }) => {
  ...
  const [currentStatus, setCurrentStatus] = useState(order.status || "Not started yet");

  const toggleDropdown = () => setIsOpen((prev) => !prev);

  const handleProgressUpdate = (newStage) => {
    setCurrentStatus(newStage); // Update local state
    const updatedOrder = { ...order, status: newStage };
    onOrderUpdate(updatedOrder); // Update Firebase
  };

  const dropdownColor = statusColors[currentStatus] || "bg-gray-100";
