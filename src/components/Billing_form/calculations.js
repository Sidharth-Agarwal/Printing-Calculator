...
const calculatePaperAndCuttingCosts = async (state) => {
  ...
  const gilCutCost = 0.25 * totalPapersRequired;
  const gilCutCostPerCard = (gilCutCost / totalCards).toFixed(2);
  ...
  return {
    paperCostPerCard: (paperCost / totalCards).toFixed(2),
    cuttingCostPerCard: (cuttingCost / totalCards).toFixed(2),
    gilCutCostPerCard: gilCutCostPerCard, // Added gil cut cost per card
    paperAndCuttingCostPerCard: ((paperCost + cuttingCost + gilCutCost) / totalCards).toFixed(2),
  };
};
...
const calculateLPCosts = async (state) => {
  ...
  if (!lpDetails.isLPUsed || !lpDetails.colorDetails?.length) {
    return { 
      lpCostPerCard: 0,
      lpPlateCostPerCard: 0,
      lpMRCostPerCard: 0,
      lpImpressionAndLaborCostPerCard: 0
    };
  }
  ...
  for (let i = 0; i < lpDetails.colorDetails.length; i++) {
    ...
    // Add to impression and labor cost
...
const calculateFSCosts = async (state) => {
  ...
  if (!fsDetails.isFSUsed || !fsDetails.foilDetails?.length) {
    return { 
      fsCostPerCard: 0,
      fsBlockCostPerCard: 0,
      fsFoilCostPerCard: 0,
      fsMRCostPerCard: 0,
      fsImpressionCostPerCard: 0
    };
  }
  ...
  for (let i = 0; i < fsDetails.foilDetails.length; i++) {
    ...
    // Add to impression cost
    totalImpressionCost += impressionCostPerCard * totalCards;
    ...
    let mrCostPerCard = totalMRRate / totalCards;
...
const calculateEMBCosts = async (state) => {
  ...
  // Sum all costs
  totalEMBCosting = totalPlateCost + mrCost;
  
  // Calculate per card cost
  const embCostPerCard = (totalEMBCosting / totalCards).toFixed(2);
  const embPlateCostPerCard = (totalPlateCost / totalCards).toFixed(2);
  const embMRCostPerCard = (mrCost / totalCards).toFixed(2);
...
const calculateSandwichCosts = async (sandwichDetails, totalCards) => {
  try {
    ...
    // Initialize all variables to store component costs
    let totalLPCostSandwich = 0;
    let lpPlateCostSandwich = 0;
    let lpMRCostSandwich = 0;
    let lpImpressionAndLaborCostSandwich = 0;
    
    let totalFSCostSandwich = 0;
    let fsBlockCostSandwich = 0;
    let fsFoilCostSandwich = 0;
    let fsMRCostSandwich = 0;
    let fsImpressionCostSandwich = 0;
    
    let totalEMBCostSandwich = 0;
    let embPlateCostSandwich = 0;
    let embMRCostSandwich = 0;
    ...
    if (lpDetailsSandwich?.isLPUsed && lpDetailsSandwich.colorDetails?.length > 0) {
      ...
      for (let i = 0; i < lpDetailsSandwich.colorDetails.length; i++) {
        ...
        // Add to impression and labor cost
        lpImpressionAndLaborCostSandwich += totalLPColorCost;
    ...
    if (fsDetailsSandwich?.isFSUsed && fsDetailsSandwich.foilDetails?.length > 0) {
      ...
      for (let i = 0; i < fsDetailsSandwich.foilDetails.length; i++) {
        ...
        // Add to impression cost
        fsImpressionCostSandwich += impressionCostPerCard * totalCards;
    ...
    if (embDetailsSandwich?.isEMBUsed) {
      ...
      // Add to plate cost
      embPlateCostSandwich = malePlateCost + femalePlateCost;
      ...
      if (mrDetails) {
        ...
        // Add to MR cost
        embMRCostSandwich = mrCost;
      } else {
        console.warn(`No MR details found for EMB MR type: ${embDetailsSandwich.embMR}`);
      }
      
      // Sum all costs
      totalEMBCostSandwich = embPlateCostSandwich + embMRCostSandwich;
    }
 
    // Calculate per card costs
    const lpCostPerCardSandwich = (totalLPCostSandwich / totalCards).toFixed(2);
    const lpPlateCostPerCardSandwich = (lpPlateCostSandwich / totalCards).toFixed(2);
    const lpMRCostPerCardSandwich = (lpMRCostSandwich / totalCards).toFixed(2);
    const lpImpressionAndLaborCostPerCardSandwich = (lpImpressionAndLaborCostSandwich / totalCards).toFixed(2);
    
    const fsCostPerCardSandwich = (totalFSCostSandwich / totalCards).toFixed(2);
    const fsBlockCostPerCardSandwich = (fsBlockCostSandwich / totalCards).toFixed(2);
    const fsFoilCostPerCardSandwich = (fsFoilCostSandwich / totalCards).toFixed(2);
    const fsMRCostPerCardSandwich = (fsMRCostSandwich / totalCards).toFixed(2);
    const fsImpressionCostPerCardSandwich = (fsImpressionCostSandwich / totalCards).toFixed(2);
    
    const embCostPerCardSandwich = (totalEMBCostSandwich / totalCards).toFixed(2);
    const embPlateCostPerCardSandwich = (embPlateCostSandwich / totalCards).toFixed(2);
    const embMRCostPerCardSandwich = (embMRCostSandwich / totalCards).toFixed(2);
 
    // Consolidate all costs
    return {
      // LP Sandwich costs
      lpCostPerCardSandwich,
      lpPlateCostPerCardSandwich,
      lpMRCostPerCardSandwich,
      lpImpressionAndLaborCostPerCardSandwich,
      
      // FS Sandwich costs
      fsCostPerCardSandwich,
      fsBlockCostPerCardSandwich,
      fsFoilCostPerCardSandwich,
      fsMRCostPerCardSandwich,
      fsImpressionCostPerCardSandwich,
      
      // EMB Sandwich costs
      embCostPerCardSandwich,
      embPlateCostPerCardSandwich,
      embMRCostPerCardSandwich
    };
  } catch (error) {
    console.error("Error calculating sandwich costs:", error);
    return { error: "Error calculating sandwich costs. Please try again." };
