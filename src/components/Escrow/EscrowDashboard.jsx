...
const EscrowDashboard = () => {
  ...
  // Process approval for a single estimate with serial number generation
  const processApproveEstimate = async (estimate, notes = "") => {
    try {
      ...
      // Keep B2B client identification
      ...
      if (clientId) {
        try {
          ...
          if (clientDoc.exists()) {
            ...
            normalizedEstimate.isLoyaltyEligible = clientData.clientType === "B2B";
          }
        } catch (error) {
      ...
      // Remove the estimate from local state
  ...
  // Handle rejection of an estimate
